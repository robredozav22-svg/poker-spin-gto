...
function renderGrid(){const grid=document.getElementById('grid');grid.innerHTML='';for(let r=0;r<13;r++)for(let c=0;c<13;c++){const cell=document.createElement('div');cell.className='hand unverified';const label=document.createElement('span');label.className='label';label.textContent=handName(r,c);cell.appendChild(label);grid.appendChild(cell);}const warning=document.getElementById('gridWarning');warning.textContent='Нет VERIFIED_EXACT hand frequencies для этого узла. Приблизительные границы не показываются.';warning.classList.remove('hidden');}
...
function renderStatus(node){const el=document.getElementById('status');const hist=state.history.length?state.history.map(x=>`${x.pos} ${x.action}`).join(' → '):'First in';const raw=node?node.status:'NO_VERIFIED_NODE';const [label,cls]=compactStatus(raw);el.innerHTML=`<div class="status-main"><strong>${state.hu?'HU':'3-MAX'} · EFF ${state.stack} BB</strong><span class="source-badge ${cls}">${label}</span></div><div>${hist}</div><div class="status-raw">${canonicalKey()} · ${raw}</div>`;}

function renderTrain(node){const p=document.getElementById('trainPrompt');const active=state.mode==='TRAIN';p.classList.toggle('hidden',!active);if(!active)return;if(!node||node.status!=='VERIFIED_EXACT'){p.innerHTML='<strong>TRAIN заблокирован для этого узла.</strong><br>Нужны VERIFIED_EXACT hand frequencies; CROSS-CHECK/APPROX не используются как ответы тренажёра.';return;}p.textContent='TRAIN ready';}
