...
enum TrainingMode{Vanilla,CfrPlusLinear}
...
impl Row{
    ...
    fn update(&mut self,a:usize,regret_delta:f64,strategy_delta:f64,plus:bool){let next=self.regret[a]+regret_delta;self.regret[a]=if plus{next.max(0.0)}else{next};self.sum[a]+=strategy_delta;}
}
...
pub struct HoldemTurnRiverCfr{
    ...
    showdown:Vec<Vec<Vec<i8>>>,training_mode:Option<TrainingMode>,iterations_completed:u64,
}
...
impl HoldemTurnRiverCfr{
    pub fn new(chance:TurnChanceTree,pot:f64,turn_bet:f64,river_bet:f64)->Result<Self,String>{
        ...
        Ok(Self{chance,pot,turn_bet,river_bet,
            ...
            r0:mk0(),r0f:mk0(),r1c:mk1(),r1f:mk1(),showdown,training_mode:None,iterations_completed:0})
    }
    ...
    pub fn iterations_completed(&self)->u64{self.iterations_completed}

    pub fn train(&mut self,iterations:usize)->Result<(),String>{self.train_internal(iterations,TrainingMode::Vanilla)}
    pub fn train_cfr_plus(&mut self,iterations:usize)->Result<(),String>{self.train_internal(iterations,TrainingMode::CfrPlusLinear)}

    fn train_internal(&mut self,iterations:usize,mode:TrainingMode)->Result<(),String>{
        if iterations==0{return Err("turn-river CFR iterations must be positive".into());}
        if let Some(existing)=self.training_mode{if existing!=mode{return Err("cannot mix vanilla CFR and CFR+ in one solver state".into());}}else{self.training_mode=Some(mode);}
        let joint=self.chance.joint_states().to_vec();let plus=mode==TrainingMode::CfrPlusLinear;
        for _ in 0..iterations{
            self.iterations_completed+=1;
            let average_weight=if plus{self.iterations_completed as f64}else{1.0};
            for s in &joint{self.cfr_turn(s.p0_index,s.p1_index,s.probability,"",1.0,1.0,plus,average_weight);}
        }
        Ok(())
    }
    ...
    pub fn evaluate_strategy(&self,s:&TurnRiverStrategy)->Result<TurnRiverValueReport,String>{
        validate_strategy(s,self.chance.p0_hands().len(),self.chance.p1_hands().len())?;let mut ev=0.0;for x in self.chance.joint_states(){ev+=x.probability*self.ev_turn(x.p0_index,x.p1_index,"",s)?;}Ok(TurnRiverValueReport{p0_value:ev,joint_private_states:self.chance.joint_states().len(),raw_river_transitions:self.chance.raw_transition_count(),normalized:true})
    }

    pub fn pure_checkdown_strategy(&self)->TurnRiverStrategy{
        let n0=self.chance.p0_hands().len();let n1=self.chance.p1_hands().len();let mk0=|row:[f64;2]|vec![vec![vec![row;n0];52];TURN_RIVER_PATHS];let mk1=|row:[f64;2]|vec![vec![vec![row;n1];52];TURN_RIVER_PATHS];
        TurnRiverStrategy{turn_p0_root:vec![[1.0,0.0];n0],turn_p0_facing_bet:vec![[1.0,0.0];n0],turn_p1_after_check:vec![[1.0,0.0];n1],turn_p1_facing_bet:vec![[1.0,0.0];n1],river_p0_root:mk0([1.0,0.0]),river_p0_facing_bet:mk0([1.0,0.0]),river_p1_after_check:mk1([1.0,0.0]),river_p1_facing_bet:mk1([1.0,0.0])}
    }
    pub fn exact_checkdown_value(&self)->f64{self.chance.exact_checkdown_showdown_sign()*(self.pot/2.0)}

    fn cfr_turn(&mut self,i:usize,j:usize,chance:f64,h:&str,rp0:f64,rp1:f64,plus:bool,average_weight:f64)->f64{
        if let Some(u)=turn_terminal(h,self.pot){return u;}
        if let Some(path)=turn_path(h){let pot2=if path==PATH_CHECK_CHECK{self.pot}else{self.pot+2.0*self.turn_bet};let a=self.chance.p0_hand(i);let b=self.chance.p1_hand(j);let board=self.chance.board();let mut out=0.0;let mut n=0;for river in 0u8..52{if board.contains(&river)||a.contains(&river)||b.contains(&river){continue;}n+=1;out+=self.cfr_river(path,i,j,river,chance/44.0,pot2,"",rp0,rp1,plus,average_weight)/44.0;}assert_eq!(n,44);return out;}
        ...
        let mut u=[0.0;2];let mut v=0.0;for a in 0..2{let next=next_history(h,a);u[a]=if player==0{self.cfr_turn(i,j,chance,next,rp0*row[a],rp1,plus,average_weight)}else{self.cfr_turn(i,j,chance,next,rp0,rp1*row[a],plus,average_weight)};v+=row[a]*u[a];}
        let target=match h{""=>&mut self.t0[i],"c"=>&mut self.t1c[j],"b"=>&mut self.t1f[j],"cb"=>&mut self.t0f[i],_=>unreachable!()};
        for a in 0..2{if player==0{target.update(a,chance*rp1*(u[a]-v),chance*rp0*row[a]*average_weight,plus);}else{target.update(a,chance*rp0*(v-u[a]),chance*rp1*row[a]*average_weight,plus);}}v
    }

    fn cfr_river(&mut self,path:usize,i:usize,j:usize,river:Card,chance:f64,pot:f64,h:&str,rp0:f64,rp1:f64,plus:bool,average_weight:f64)->f64{
        let cmp=self.showdown[river as usize][i][j];if let Some(u)=river_terminal(cmp,h,pot,self.river_bet){return u;}let r=river as usize;
        let (player,row)=match h{""=>(0,self.r0[path][r][i].current()),"c"=>(1,self.r1c[path][r][j].current()),"b"=>(1,self.r1f[path][r][j].current()),"cb"=>(0,self.r0f[path][r][i].current()),_=>panic!("invalid river history {h}")};
        let mut u=[0.0;2];let mut v=0.0;for a in 0..2{let next=next_history(h,a);u[a]=if player==0{self.cfr_river(path,i,j,river,chance,pot,next,rp0*row[a],rp1,plus,average_weight)}else{self.cfr_river(path,i,j,river,chance,pot,next,rp0,rp1*row[a],plus,average_weight)};v+=row[a]*u[a];}
    ...
    fn ev_turn(&self,i:usize,j:usize,h:&str,s:&TurnRiverStrategy)->Result<f64,String>{
        if let Some(u)=turn_terminal(h,self.pot){return Ok(u);}if let Some(path)=turn_path(h){let pot2=if path==PATH_CHECK_CHECK{self.pot}else{self.pot+2.0*self.turn_bet};let a=self.chance.p0_hand(i);let b=self.chance.p1_hand(j);let board=self.chance.board();let mut out=0.0;let mut n=0;for river in 0u8..52{if board.contains(&river)||a.contains(&river)||b.contains(&river){continue;}n+=1;out+=self.ev_river(path,i,j,river,pot2,"",s)?/44.0;}if n!=44{return Err("turn evaluator did not enumerate exactly 44 legal rivers".into());}return Ok(out);}let row=match h{""=>s.turn_p0_root[i],"c"=>s.turn_p1_after_check[j],"b"=>s.turn_p1_facing_bet[j],"cb"=>s.turn_p0_facing_bet[i],_=>return Err("invalid turn evaluation history".into())};Ok(row[0]*self.ev_turn(i,j,next_history(h,0),s)?+row[1]*self.ev_turn(i,j,next_history(h,1),s)?)
    }
    fn ev_river(&self,path:usize,i:usize,j:usize,river:Card,pot:f64,h:&str,s:&TurnRiverStrategy)->Result<f64,String>{let cmp=self.showdown[river as usize][i][j];if let Some(u)=river_terminal(cmp,h,pot,self.river_bet){return Ok(u);}let r=river as usize;let row=match h{""=>s.river_p0_root[path][r][i],"c"=>s.river_p1_after_check[path][r][j],"b"=>s.river_p1_facing_bet[path][r][j],"cb"=>s.river_p0_facing_bet[path][r][i],_=>return Err("invalid river evaluation history".into())};Ok(row[0]*self.ev_river(path,i,j,river,pot,next_history(h,0),s)?+row[1]*self.ev_river(path,i,j,river,pot,next_history(h,1),s)?) }
}

fn precompute_showdowns(chance:&TurnChanceTree)->Vec<Vec<Vec<i8>>>{let n0=chance.p0_hands().len();let n1=chance.p1_hands().len();let q=chance.board();let mut out=vec![vec![vec![0i8;n1];n0];52];for river in 0u8..52{if q.contains(&river){continue;}for i in 0..n0{let a=chance.p0_hand(i);if a.contains(&river){continue;}for j in 0..n1{let b=chance.p1_hand(j);if b.contains(&river)||!disjoint(a,b){continue;}let x=evaluate_seven([a[0],a[1],q[0],q[1],q[2],q[3],river]);let y=evaluate_seven([b[0],b[1],q[0],q[1],q[2],q[3],river]);out[river as usize][i][j]=if x>y{1}else if x<y{-1}else{0};}}}out}
...
mod tests{
    ...
    #[test]fn cfr_plus_strategy_is_normalized(){let mut g=game();g.train_cfr_plus(100).unwrap();assert!(g.evaluate_average().unwrap().p0_value.is_finite());}
    #[test]fn mixing_training_modes_is_rejected(){let mut g=game();g.train(10).unwrap();assert!(g.train_cfr_plus(10).is_err());}
}
