...
pub fn evaluate_independent_br(
    chance:&TurnChanceTree,pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy,current_p0_value:f64,
)->Result<TurnRiverBrReport,String>{
    ...
    let p0_br_gain=(p0_br-current_p0_value).max(0.0);
    let p1_br_gain=(current_p0_value-vs_p1_br).max(0.0);
...
fn p0_best_response(chance:&TurnChanceTree,pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    let n0=chance.p0_hands().len();let n1=chance.p1_hands().len();let mut joint=vec![vec![0.0;n1];n0];
    for x in chance.joint_states(){joint[x.p0_index][x.p1_index]=x.probability;}
    let mut total=0.0;
    for i in 0..n0{
        let mut opp=vec![0.0;n1];for j in 0..n1{opp[j]=joint[i][j];}
...
fn p0_turn_root_check(chance:&TurnChanceTree,i:usize,opp:&[f64],pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    ...
    let check_cont=p0_river_br(chance,i,&checked,pot,river_bet,s);
    let fold=-pot/2.0*bet.iter().sum::<f64>();
    let call=p0_river_br(chance,i,&bet,pot+2.0*turn_bet,river_bet,s);
...
fn p0_turn_root_bet(chance:&TurnChanceTree,i:usize,opp:&[f64],pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    ...
    pot/2.0*fold_mass+p0_river_br(chance,i,&called,pot+2.0*turn_bet,river_bet,s)
}

fn p0_river_br(chance:&TurnChanceTree,i:usize,opp:&[f64],pot:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    let hero=chance.p0_hand(i);let board=chance.board();let mut total=0.0;
    for river in 0u8..52{
        if board.contains(&river)||hero.contains(&river){continue;}
        let mut legal_mass=0.0;for j in 0..opp.len(){let v=chance.p1_hand(j);if !v.contains(&river)&&!overlap(hero,v){legal_mass+=opp[j];}}
        if legal_mass<=f64::EPSILON{continue;}
        let r=river as usize;
        let mut check_show=0.0;let mut after_check_bet=vec![0.0;opp.len()];
        for j in 0..opp.len(){let v=chance.p1_hand(j);if opp[j]<=0.0||v.contains(&river)||overlap(hero,v){continue;}let w=opp[j]/44.0;let cmp=cmp(chance,hero,v,river);check_show+=w*s.river_p1_after_check[r][j][0]*show(cmp,pot,false,river_bet);after_check_bet[j]=w*s.river_p1_after_check[r][j][1];}
        ...
        for j in 0..opp.len(){let v=chance.p1_hand(j);if opp[j]<=0.0||v.contains(&river)||overlap(hero,v){continue;}let w=opp[j]/44.0;let row=s.river_p1_facing_bet[r][j];root_bet+=w*(row[0]*(pot/2.0)+row[1]*show(cmp(chance,hero,v,river),pot,true,river_bet));}
...
fn p1_after_turn_check(chance:&TurnChanceTree,j:usize,hero_weights:&[f64],pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    let check=p1_river_br(chance,j,hero_weights,pot,river_bet,s);
    ...
    let bet=fold_value+p1_river_br(chance,j,&called,pot+2.0*turn_bet,river_bet,s);
    check.min(bet)
}
...
fn p1_facing_turn_bet(chance:&TurnChanceTree,j:usize,hero_weights:&[f64],pot:f64,turn_bet:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    ...
    let call=p1_river_br(chance,j,hero_weights,pot+2.0*turn_bet,river_bet,s);
    fold.min(call)
}

fn p1_river_br(chance:&TurnChanceTree,j:usize,hero_weights:&[f64],pot:f64,river_bet:f64,s:&TurnRiverStrategy)->f64{
    let villain=chance.p1_hand(j);let board=chance.board();let mut total=0.0;
    for river in 0u8..52{
        if board.contains(&river)||villain.contains(&river){continue;}
        let r=river as usize;
        let mut after_p0_check=vec![0.0;hero_weights.len()];let mut facing_p0_bet=vec![0.0;hero_weights.len()];
        for i in 0..hero_weights.len(){let h=chance.p0_hand(i);if hero_weights[i]<=0.0||h.contains(&river)||overlap(h,villain){continue;}let w=hero_weights[i]/44.0;after_p0_check[i]=w*s.river_p0_root[r][i][0];facing_p0_bet[i]=w*s.river_p0_root[r][i][1];}
        ...
        let mut p1_bet=0.0;for i in 0..hero_weights.len(){if after_p0_check[i]<=0.0{continue;}let row=s.river_p0_facing_bet[r][i];let c=cmp(chance,chance.p0_hand(i),villain,river);p1_bet+=after_p0_check[i]*(row[0]*(-pot/2.0)+row[1]*show(c,pot,true,river_bet));}
...
fn cmp(chance:&TurnChanceTree,a:Combo,b:Combo,river:Card)->i8{
    let q=chance.board();let x=evaluate_seven([a[0],a[1],q[0],q[1],q[2],q[3],river]);let y=evaluate_seven([b[0],b[1],q[0],q[1],q[2],q[3],river]);if x>y{1}else if x<y{-1}else{0}
}
fn show(cmp:i8,pot:f64,called:bool,bet:f64)->f64{cmp as f64*(pot/2.0+if called{bet}else{0.0})}
...
fn validate_strategy_shape(chance:&TurnChanceTree,s:&TurnRiverStrategy)->Result<(),String>{
    ...
    if s.river_p0_root.len()!=52||s.river_p0_facing_bet.len()!=52||s.river_p1_after_check.len()!=52||s.river_p1_facing_bet.len()!=52{return Err("two-street BR river slot mismatch".into());}
    Ok(())
}
...
mod tests{
    ...
    #[test]fn best_response_bounds_current_value_after_training(){let (t,mut g)=fixture();g.train(1000).unwrap();let v=g.evaluate_average().unwrap().p0_value;let s=g.average_strategy();let r=evaluate_independent_br(&t,4.0,4.0,8.0,&s,v).unwrap();assert!(r.p0_best_response_value+1e-9>=v);assert!(r.p0_value_vs_p1_best_response-1e-9<=v);assert!(r.nashconv>=0.0);}
}
