...
fn main(){
    ...
    assert_eq!(game1.pairs.len(),legal_pairs);assert_eq!(game1.equity_cache.misses(),0);

    let sweeps=10_000usize;
    let solve1_start=Instant::now();for _ in 0..sweeps{game1.sweep().unwrap();}
    ...
    let independent=evaluate_btn_fold_sb_jam_bb_call_exact(
        8.0,&sb_prior,&bb_prior,&sb1,&bb1,&blockers,&lookup
    ).expect("independent 24x24 exact evaluation");
    let tol=1e-12;
    ...
    let sb2=game2.sb_average_strategy();let bb2=game2.bb_average_strategy();let report2=game2.evaluate(&sb2,&bb2).unwrap();
    ...
    println!(
        ...
        sweeps,solve1_seconds,solve2_seconds,sweeps as f64/solve1_seconds,avg_sb_jam,avg_bb_call,report1.nashconv,
        independent.normalized_nashconv_fraction_of_pot,report1.sb_br_gain,report1.bb_br_gain,report1.sb_value
    );
    println!("NOTE: bounded 24x24 physical-combo restricted game only; not full Spin GTO and not chart data. Passing the <=0.01% pot internal target here does NOT authorize a full-Spin or GTO-Wizard superiority claim.");
}
