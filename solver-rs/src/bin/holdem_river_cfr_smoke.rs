...
fn main(){
    ...
    assert!(report.nashconv<0.0006,"river NashConv too large: {}",report.nashconv);
    println!(
        "status=RESEARCH_ONLY mode=HOLDEM_RIVER_RANGE_CFR iterations={} board_cards=5 p0_support=4 p1_support=4 legal_joint_states={} pot_bb={:.3} bet_bb={:.3} current_p0_value={:.12} p0_br={:.12} p0_vs_p1_br={:.12} p0_br_gain={:.12} p1_br_gain={:.12} nashconv={:.12} nashconv_max=0.000600000000 joint_normalizer={:.12} holdem_evaluator=EXACT_SEVEN_CARD blockers=EXACT independent_bruteforce_br=PASS gate=PASS",
