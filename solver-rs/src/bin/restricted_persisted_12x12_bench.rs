...
fn main(){
    // Bounded deterministic physical-combo support. This is deliberately not a
    // 169-class range and not a claim about real Spin strategy composition.
    let sb=[
    ...
    let (table,stats)=build_hu_payoff_table(&requested,None).expect("12x12 exact payoff build");
    ...
    let mut game1=ExactRestrictedSubgame::new_from_payoff_lookup(8.0,sb_prior.clone(),bb_prior.clone(),&lookup).expect("persisted 12x12 game 1");
    ...
    let mut game2=ExactRestrictedSubgame::new_from_payoff_lookup(8.0,sb_prior,bb_prior,&lookup).expect("persisted 12x12 game 2");
    ...
    assert_eq!(sb1,sb2,"repeated 12x12 persisted solve SB strategy differs");
    assert_eq!(bb1,bb2,"repeated 12x12 persisted solve BB strategy differs");
    assert_eq!(report1,report2,"repeated 12x12 persisted solve report differs");
    ...
    println!(
        "status=RESEARCH_ONLY mode=PERSISTED_RESTRICTED_12X12 support_sb={} support_bb={} legal_pairs={} unique_canonical_payoffs={} artifact_bytes={} payoff_build_seconds={:.6} payoff_keys_per_second={:.6} lookup_construct_seconds={:.6} sweeps={} solve1_seconds={:.6} solve2_seconds={:.6} solve_sweeps_per_second={:.3} avg_sb_jam={:.9} avg_bb_call={:.9} nashconv={:.9} sb_br_gain={:.9} bb_br_gain={:.9} sb_value={:.9} exact_enumeration_during_solve=ZERO deterministic_repeat=PASS",
        sb.len(),bb.len(),legal_pairs,requested.len(),artifact_bytes,build_seconds,requested.len() as f64/build_seconds,construct_seconds,
    ...
    println!("NOTE: bounded 12x12 physical-combo restricted game only; not full Spin GTO and not chart data.");
}
