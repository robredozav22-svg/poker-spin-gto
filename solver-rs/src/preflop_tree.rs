...
pub struct PreflopNodeKey{pub format:GameFormat,pub payout:PayoutProfile,pub tree_profile_id:String,pub effective_stack:Bb100,pub actor:Player,pub history:Vec<HistoryEvent>}
impl PreflopNodeKey{
...
pub struct ActionEdge{pub action:PreflopAction,pub continuation:ContinuationContract,pub next_actor:Option<Player>}
impl ActionEdge{
...
impl PreflopDecisionSpec{
    pub fn new(key:PreflopNodeKey,edges:Vec<ActionEdge>,evidence:TreeEvidence)->Result<Self,String>{
        ...
        for edge in &edges{validate_action_amount(edge.action,key.effective_stack)?;if !seen.insert(edge.action){return Err(format!("duplicate action in decision node: {:?}",edge.action));}validate_continuation_contract(edge,key.effective_stack)?;}
    ...
    pub fn child_key(&self,edge_index:usize)->Result<PreflopNodeKey,String>{
        ...
        let mut history=self.key.history.clone();history.push(HistoryEvent{actor:self.key.actor,action:edge.action});
...
fn validate_continuation_contract(edge:&ActionEdge,stack:Bb100)->Result<(),String>{
    ...
    match (edge.action,edge.continuation){
        // In multiway poker a fold can either end the hand, pass the preflop turn,
        // or remove one player while remaining players continue postflop.
        (PreflopAction::Fold,ContinuationContract::ExactFoldSettlement|ContinuationContract::ChildDecision|ContinuationContract::RequiresPostflopEv|ContinuationContract::Unresolved)=>Ok(()),
...
mod tests{
    ...
    #[test] fn multiway_fold_can_require_postflop_ev(){let key=PreflopNodeKey::new(GameFormat::Spin3Max,PayoutProfile::WinnerTakeAllChipEv,PROFILE,Bb100(1500),Player::Bb,vec![HistoryEvent{actor:Player::Btn,action:PreflopAction::RaiseTo(Bb100(200))},HistoryEvent{actor:Player::Sb,action:PreflopAction::CallTo(Bb100(200))}]).unwrap();let spec=PreflopDecisionSpec::new(key,vec![ActionEdge::terminal(PreflopAction::Fold,ContinuationContract::RequiresPostflopEv),ActionEdge::terminal(PreflopAction::CallTo(Bb100(200)),ContinuationContract::RequiresPostflopEv)],TreeEvidence::new(TreeVerification::ScreenReference,"fixture").unwrap()).unwrap();assert_eq!(spec.edges[0].continuation,ContinuationContract::RequiresPostflopEv);}
